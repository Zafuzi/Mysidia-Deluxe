...
class MyadoptsController extends AppController{
	...
	public function manage(){
		...
		$this->setField("name", new Str($this->adopt->getName()));	
	...
	*/
		...
		$this->setField("aid", new Integer($this->adopt->getAdoptID()));
		$this->setField("image", $this->image);				
		$this->setField("message", new Str($message));				
	}
	...
	}		public function raise(){		$this->setField("adopt", $this->adopt);		$mysidia = Registry::get('mysidia');		if ($mysidia->input->post('submit') != null){						$cost = 300 + (20 * $mysidia->input->post("number"));						if($cost <= $mysidia->user->money){				$moneyleft = $mysidia->user->money - $cost;				$mysidia->db->update('users', ['money'=>$moneyleft], "uid={$mysidia->user->uid}");				$this->adopt->setRaise($mysidia->input->post("number"), "update");			}						else{				$message = 'You cannot afford this much. The total cost would be '.number_format($cost).' beads. <a href="'.$this->adopt->aid.'">Return to try again</a>';				throw new Exception($message);							}		}	}
}