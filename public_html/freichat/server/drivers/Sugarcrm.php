...
class Sugarcrm extends driver_base {
...
//------------------------------------------------------------------------------
    public function getDBdata($session_id, $first) {
        if ($_SESSION[$this->uid . 'time'] < $this->online_time || isset($_SESSION[$this->uid . 'usr_name']) == false || $first == 'false') {   //To consume less resources , now the query is made only once in 15 seconds
            $query = "SELECT DISTINCT j.user_name,j.id
                      FROM " . DBprefix . "users AS j
                      WHERE j.id=?
                      LIMIT 1";
            ...
            foreach ($res as $result) {
                ...
                $_SESSION[$this->uid . 'usr_ses_id'] = $_SESSION[$this->uid . 'gst_ses_id'];//$result['id']; To avoid characters
                //$_SESSION['fg45ty45g'] = 223;
            }
        }
    }
