...
// Note: this file is not referenced or used in the public source code of 0h n0
...
var PlayCenter = new (function() {
	...
	// enabled = true; platform = Platform.Apple; isSignedIn = true;
	// window.isWebApp = false;
	// $('html').addClass('playcenter xsigned-in');
	...
	this.IDS = {
		Leaderboards: {
		  ...
		  'games_played': 						{ id: '', 1: '', 2: '' },
		  'best_time': 								{ id: '', 1: '', 2: '' },
		  '_5_x_5_played': 						{ id: '', 1: '', 2: '' },
		  '_6_x_6_played': 						{ id: '', 1: '', 2: '' },
		  '_7_x_7_played': 						{ id: '', 1: '', 2: '' },
		  '_8_x_8_played': 						{ id: '', 1: '', 2: '' },
		  'best_time_5_x_5': 					{ id: '', 1: '', 2: '' },
		  'best_time_6_x_6': 					{ id: '', 1: '', 2: '' },
		  'best_time_7_x_7': 					{ id: '', 1: '', 2: '' },
		...
		Achievements: {
		  ...
		  'no_questions_asked_5_x_5': { id: '', 1: '', 2: '' },
		  'no_questions_asked_6_x_6': { id: '', 1: '', 2: '' },
		  'no_questions_asked_7_x_7': { id: '', 1: '', 2: '' },
		  'no_questions_asked_8_x_8': { id: '', 1: '', 2: '' },
		  'forward_5_x_5': 						{ id: '', 1: '', 2: '' },
		  'forward_6_x_6': 						{ id: '', 1: '', 2: '' },
		  'forward_7_x_7': 						{ id: '', 1: '', 2: '' },
	...
	this.signIn = function (auto) {
		...
		function success(obj) {
			...
			if (platform == Platform.Apple) {
				...
				$('#menu #bar').html('');
				//alert('signed in, resetting stuff now...')
		...
		function fail(obj) {
			...
			if (!auto && /error/.test(errorMsg.toLowerCase(), 'gi')) {
				if (platform == Platform.Apple) {
					Game.showMessage('<p>Signing in with Game Center didn\'t work.</p><p>Please check your internet connection.</p><p>If this problem persists, open the Settings app, go to Game Center and sign in there.</p><p>If that doesn\'t work, give up and go play 0h h1 :)</p>')
				}
				else if (platform == Platform.Android) {
					Game.showMessage('<p>Signing in with Google Play Game Services didn\'t work.</p><p>Please check your internet connection.</p><p>If this problem persists, try again later or give up and go play 0h h1 :)</p>')
				}
			}
	...
	this.submitScore = function(leaderboardObj, score) {
		...
		if (enabled && isSignedIn && id) {
			...
			if (platform == Platform.Apple) {
				...
				gameCenter.reportScore(id, score, function(obj) {
	...
	this.unlockAchievement = function (achievementObj) {
		...
		if (enabled && isSignedIn && id) {
			...
			if (platform == Platform.Apple && id) {
				gameCenter.reportAchievement(id, success, function(obj) {
