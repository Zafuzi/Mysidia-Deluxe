...
class explorebathroomview extends View{
	...
	public function index(){
		...
		if($hasitem){
        	...
        	$document->setTitle("<center>Your pet {$pet->name} is taking a bath<br>"); 
            ...
            $document->add(new Comment("<center><a href='http://atrocity.mysidiahost.com/explorebathroom'>Give pet another bath</a><br><br></center>", FALSE));
	
			//They have the item, so they can explore now
			$document->add(new Comment("<center><img src='https://cdn.discordapp.com/attachments/502505548818939941/567456472263295036/takingabahgif1.gif'></center><br/>")); 
	
                $boost = mt_rand(10,50);
                if($pet->getHappiness() < 50){
                    $new_mood = $pet->getHappiness() + $boost; if($new_mood > 50){$new_mood = 50;}
                    $mysidia->db->update("adoptable_stats", array("happiness" => $new_mood), "id = '$pet->aid'");
                    $document->add(new Comment("<center>That was a wonderful bath! <b>{$pet->name}</b> has recovered <b>{$boost}</b> happiness.</center>"));
                }
                else{$document->add(new Comment("<center>That was a wonderful bath!</center>"));}
		}
		else {
		    $document->setTitle("<center>Whoops!</center><br>");
