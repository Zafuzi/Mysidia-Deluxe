...
class Explorefarmmeadowview extends View{
	
	public function index(){
	    $mysidia = Registry::get("mysidia");
		$document = $this->document;
        	$document->setTitle("<center>Exploring the farm meadow<br>"); 
        	$document->add(new Comment("<a href='http://atrocity.mysidiahost.com/pages/view/meadow/'>Go back</a><br><br></center>", FALSE));
        	        			      	$document->add(new Comment("<a href='http://atrocity.mysidiahost.com/explorefarmmeadow'>Search again?</a><br><br></center>", FALSE));
		...
		if($hasitem){
			...
			if($random > 1 && $random < 5){
				$item = "Mini heart rivvid";
				$qty = 1;
				$newitem = new StockItem($item);
				$newitem->append($qty, $mysidia->user->username);  
				$document->add(new Comment("<img src='http://atrocity.mysidiahost.com/picuploads/png/353425a50030f473f2246f485b89a83e.png'>Oh look! You found a <b>{$item}</b> Companion!", FALSE));
			}
			...
			elseif($random >= 16 && $random <= 25){
				$item = "Mini Himalayan Rivvid";
				$qty = 1;
				$newitem = new StockItem($item);
				$newitem->append($qty, $mysidia->user->username);  
				$document->add(new Comment("<img src='http://atrocity.mysidiahost.com/picuploads/png/e2c9267f9b58b1cc3937d660cac02b69.png'>Oh look! You found a <b>{$item}</b>!", FALSE));
			}
			...
			elseif($random >= 41 && $random <= 50){
				$item = "Mini Black speckled Rivvid";
				$qty = 1;
				$newitem = new StockItem($item);
				$newitem->append($qty, $mysidia->user->username);  
				$document->add(new Comment("<img src='http://atrocity.mysidiahost.com/picuploads/png/7ea46153fbfb832e2bad53dbf15a2425.png'>Oh look! You found a <b>{$item}</b>!", FALSE));
			}
